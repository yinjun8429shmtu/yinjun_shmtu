%% objective£ºsum1~V£¨||X_v-U_vP||^2+beta*||U_v||^2£©+alpha*tr(PLwP')+gama*||W||^2
...
function [data,bas,new_data,J,W] = IMCRV-1(data_e,ori_new_data,ori_bas,los_mark,options)
...
for it_mark = 1:maxiter  
    ...
    DIS = alpha*(tempW1+tempW2-2*tempW3)/(2*gama); 
    sort_DIS = sort(DIS);
    for data_mark = 1:data_num
        for data_mark2 = 1:data_num   
           tempW4 = (sum(sort_DIS(1:data_mark2,data_mark))+1)/data_mark2 - sort_DIS(data_mark2,data_mark); 
           if tempW4 <= 0
                break;
           end
        end
        k = data_mark2 - 1;
        W(:,data_mark) = repmat((sum(sort_DIS(1:k,data_mark))+1)/k,data_num,1) - DIS(:,data_mark);
    end
    W = max(W,0);
    %%%%
    ...
    J(it_mark) = J(it_mark) + alpha*trace(new_data*LL*new_data')+gama*trace(W'*W);
